# Status projektu — zatrzymane na życzenie użytkownika (01.09.2026)
...
w niedokończonym stanie na dysku roboczym. Gałąź `main` jest w pełni wypchnięta
do `origin/main` i spójna.
...
- **Silnik 3D**: three.js r180 wgrany do `assets/vendor/` (nie z CDN), `assets/sky3d.js`
  ...
  we wcześniejszej historii) — widgety renderują na żądanie, nie w pętli. Awaria widgetu
  jest widoczna: `.viz3d__fallback` dostaje linię `.viz3d__why` z przyczyną (commit 7b43c4e).
...
**Domknięte do progu 2000+ słów (17 z 24):** R.1, R.2, R.3, R.4, R.5, R.9, R.10, R.11,
R.13, R.14, R.15, R.17, R.19, R.20, R.21, R.23, R.24.

**Wciąż poniżej 2000 słów (7 z 24) — to jest następny krok:**
...
Uwaga: liczby w tabeli pochodzą z pierwotnego pomiaru licznikiem, który liczył wyłącznie
prozę w `.section`. Licznik nie jest zapisany jako trwały plik — trzeba go odtworzyć
w nowej sesji, inaczej wyniki nie będą porównywalne z tą tabelą.

Metoda sprawdzona na 17 gotowych rozdziałach: dopisuję 1–3 nowe `.section`, każda
...
Ostatnie partie: partia 6 (b015366) — R.1 i R.2; partia 7 (558ab2e) — R.3 (polaryzacja
błękitu, niebo jako drugie źródło światła) i R.4 (zjawisko Purkiniego).

## Wzorzec do rozważenia: przyciski "Zaawansowane"

Commit 11572e0 dodał do Rozdziału 2 cztery przyciski "Zaawansowane" — rozwijają
pogłębienie przy miejscach, gdzie tekst główny świadomie upraszcza. Wzorzec żyje na
razie tylko w R.2 i w `assets/style.css`. Jeśli ma zostać, warto go rozprowadzić po
pozostałych rozdziałach; jeśli nie — usunąć z R.2, żeby jedna strona nie odstawała.
Decyzja nie została podjęta.

## Co zostało do zrobienia

1. **Dokończyć przebieg wzbogacający** na 7 rozdziałach z tabeli wyżej — to jedyna
   praca merytoryczna, jaka pozostała. Reszta książki (dodatki, strony terenowe,
   index) jest kompletna i nie wymaga zmian.
2. **Rozstrzygnąć los przycisków "Zaawansowane"** (sekcja wyżej).
3. **Promocja `assets/sky3d.js` do `learning-materials`** — nietknięta. Tamta sesja miała
...
4. **GitHub Pages nie jest włączone** — repo jest prywatne, `.nojekyll` czeka w root.
   Do ogłoszenia trzeba włączyć Pages z `main` / root.
5. Nic więcej nie jest zaplanowane — po punkcie 1 książka jest gotowa do ogłoszenia.

## Jak wznowić

Wystarczy poprosić o kontynuację przebiegu wzbogacającego, wskazując rozdział z tabeli
wyżej albo mówiąc "zrób kolejną partię". Narzędzia (`dev/dopisz.py`, `dev/wstaw.py`
i `dev/scaffold.py sprawdz`) są w repo; licznik słów i `tchk.mjs` trzeba odtworzyć —
są proste (patrz historia commitów za wzorzec sekcji).
