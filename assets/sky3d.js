/**
 * sky3d.js — silnik widgetów 3D dla "Atmosfera i chmury dla ciekawych".
 *
 * Filozofia skopiowana z viz.js z learning-materials: funkcje fabryczne biorą
 * element DOM i obiekt konfiguracji, zwracają { ok, set(patch), render, dispose }.
 * Żadna z nich nie zakłada niczego o stronie, na której stoi.
 *
...
export function createScatterLobe(host, cfg = {}) {
  ...
  labels.add('światło pada', new THREE.Vector3(-2.7, 0.3, 0), { color: t.accent });
  labels.add('do przodu', new THREE.Vector3(2.6, 0.28, 0), { color: t.muted });
...
export function createDiagram3D(host, cfg = {}) {
  ...
  if (!st) return { ok: false, set() {}, dispose() {}, setLayer() {} };
  const t = st.theme;
...
/**
 * Jedno wejście dla stron: znajduje .viz3d[data-widget] i uruchamia właściwą
 * fabrykę. Strona nie musi nic importować poza tą funkcją, a widget, którego
 * nie da się uruchomić, po prostu pokazuje fallback.
 */
