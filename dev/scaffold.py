...
Robi dwie rzeczy:
  ...
  python3 dev/scaffold.py szkielet   — tworzy BRAKUJĄCE strony z pełnym
  ...
  python3 dev/scaffold.py sprawdz    — kontrola spójności: martwe linki,
        zgodność nawigacji górnej z dolną, obustronna zgodność EXT OF
        z blokami "Idź głębiej", brakujące bloki obowiązkowe.

Przy pięćdziesięciu trzech stronach z ręcznie utrzymywaną nawigacją
"sprawdz" to jedyny sposób, żeby złapać literówkę w linku "Następny →".
"""
...
def teren_page(t):
    links = [('← Spis treści', '../index.html')]
    return (
        head(f'{t["tytul"]} — {SPIS["tytul"]}')
        + topnav(links)
        + '\n<div class="page">\n\n'
        + readout(['W terenie', 'Na telefon', 'Bez internetu', 'Patrz w górę'])
        + '\n  <div class="eyebrow">W terenie</div>\n'
        + f'  <h1>{t["tytul"]}</h1>\n'
        + f'  <p class="subtitle">{t["opis"]}</p>\n\n'
        + '  <!-- TRESC -->\n\n'
        + '  <div class="site-nav">\n'
        + '    <a href="../index.html">← Spis treści</a>\n'
        + '  </div>\n\n'
...
def cmd_szkielet():
    ...
    for d in SPIS['dodatki']:
        ...
        if os.path.exists(path):
            skipped.append(d['slug']); continue
        open(path, 'w', encoding='utf-8').write(dodatek_page(d, ch_by_nr))
        made.append(d['slug'])

    for t in SPIS['teren']:
        path = os.path.join(ROOT, 'teren', t['slug'] + '.html')
        if os.path.exists(path):
            skipped.append(t['slug']); continue
        open(path, 'w', encoding='utf-8').write(teren_page(t))
        made.append(t['slug'])
...
def all_pages():
    pages = []
    for sub in ('rozdzialy', 'dodatki', 'teren'):
...
def cmd_sprawdz():
    ...
    for i, ch in enumerate(rozdz):
        ...
        if not os.path.exists(path):
            problems.append(f'BRAK PLIKU   rozdzialy/{ch["slug"]}.html'); continue
        html = open(path, encoding='utf-8').read()
    ...
    for d in SPIS['dodatki']:
        path = os.path.join(ROOT, 'dodatki', d['slug'] + '.html')
        if not os.path.exists(path):
            problems.append(f'BRAK PLIKU   dodatki/{d["slug"]}.html'); continue
        html = open(path, encoding='utf-8').read()
